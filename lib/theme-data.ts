...
export async function getRemoteThemeData(): Promise<{
    ...
    try {
        const [colorsRes, infoRes, metaRes] = await Promise.all([
            fetch(`${THEME_API_URL}/api/theme-colors?domain=${domain}`, { headers, next: { revalidate: 0 } }),
            fetch(`${THEME_API_URL}/api/store-info?domain=${domain}`, { headers, next: { revalidate: 0 } }),
            fetch(`${THEME_API_URL}/api/store-metadata?domain=${domain}`, { headers, next: { revalidate: 0 } })
        ]);
        ...
        if ((!storeInfo || !metadata || !colors) && domain !== cleanDomain) {
            const [cRes, iRes, mRes] = await Promise.all([
                !colors ? fetch(`${THEME_API_URL}/api/theme-colors?domain=${cleanDomain}`, { headers, next: { revalidate: 0 } }) : Promise.resolve(null),
                !storeInfo ? fetch(`${THEME_API_URL}/api/store-info?domain=${cleanDomain}`, { headers, next: { revalidate: 0 } }) : Promise.resolve(null),
                !metadata ? fetch(`${THEME_API_URL}/api/store-metadata?domain=${cleanDomain}`, { headers, next: { revalidate: 0 } }) : Promise.resolve(null)
            ]);
