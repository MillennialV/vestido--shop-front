...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        if (isGarmentRoute) {
          const garmentSlug = path.substring(1);
          const garmentToOpen = fetchedGarments.find(
            (g) => g.slug === garmentSlug,
          );
          setSelectedGarment(garmentToOpen || null);
          if (garmentToOpen) {
            fetchProductById(garmentToOpen.id).then((fresh) => {
  ...
  const handleSaveGarment = async (product: Garment) => {
    console.log("[App] handleSaveGarment llamado con:", {
      id: product.id,
      title: product.title,
    });
    try {
      ...
      console.log(
        `Error al procesar la prenda: ${getFriendlySupabaseError(err)}`,
      );
    }
  };
  ...
  const handleDeleteGarment = async (id: number) => {
    ...
    try {
      ...
      console.log(
        `Error al eliminar la prenda: ${getFriendlySupabaseError(err)}`,
      );
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      ...
      try {
        ...
        console.log(
          `Error al eliminar las prendas: ${getFriendlySupabaseError(err)}`,
        );
      }
    }
  };
  ...
  const handleReorderFaqs = useCallback(
    async (newOrder: FaqItem[], result: DropResult) => {
  ...
  const renderPage = () => {
    if (currentPath.startsWith("/blog/")) {
      ...
      return (
        <div className="text-center py-16 text-stone-900 dark:text-stone-100">
