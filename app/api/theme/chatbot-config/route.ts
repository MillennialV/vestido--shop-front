...
export async function GET(req: NextRequest) {
    ...
    try {
        const url = new URL(`${THEME_API_URL}/api/chatbot-config`);
        ...
        if (!backendRes.ok && backendRes.status === 404 && domain !== cleanDomain && !orgId) {
            console.log(`[api/theme/chatbot-config] 404 with ${domain}, trying with ${cleanDomain}`);
        ...
        if (!backendRes.ok) {
            ...
            console.error("Backend Chatbot Config GET Error:", { status: backendRes.status, body: errorText });
        ...
        console.error("Error en GET /api/theme/chatbot-config:", err);
...
export async function POST(req: NextRequest) {
    try {
        ...
        const backendRes = await fetch(`${THEME_API_URL}/api/chatbot-config`, {
            method: "POST",
            headers: {
        ...
        console.error("Error en POST /api/theme/chatbot-config:", err);
