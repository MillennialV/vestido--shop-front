import { Post, PostResponse, BlogPagination } from "@/types/post";
...
class PostService {
  ...
  async getCategories(): Promise<Category[]> {
    ...
    if (!res.success) throw new Error(res.message || res.error || 'Error al obtener categorías');
    return res.data;
  }

  async createPost(post: Omit<Post, 'id' | 'slug' | 'created_at' | 'updated_at'>): Promise<Post> {
    const url = `${BLOG_BASE_API}/api/blog/posts`;
    const res = await apiResponse<Post>(url, { method: "POST", body: JSON.stringify(post) });
    if (!res.success) throw new Error(res.message || res.error || 'Error al crear el post');
    return res.data;
  }

  async readPosts(params: ApiParams = {}): Promise<{ posts: Post[]; pagination: BlogPagination }> {
  ...
  async readPost(id: number | string): Promise<Post> {
    ...
    if (!res.success) throw new Error(res.message || res.error || 'Error al obtener el post');
    return res.data;
  }
  ...
  async updatePost(id: number | string, data: Partial<Post>): Promise<Post> {
    ...
    if (!res.success) throw new Error(res.message || res.error || 'Error al actualizar el post');
    return res.data;
  }
